...
namespace Eliot.AgentComponents
{
    ...
    public class StandardInventoryConditionInterface : InventoryConditionInterface
    {
        ...
        {
            // Pastikan memanggil fungsi yang sudah diupdate tipe datanya
            return AgentInventory.HaveBetterItem(Eliot.Environment.ItemType.Weapon);
        }
        ...
        public bool HaveHealingPotion()
        {
            foreach (var item in AgentInventory.Items)
                if (item.Type.Equals(Eliot.Environment.ItemType.HealingPotion) && item.Skill)
                {
                    return true;
                }
            return false;
        ...
        {
            foreach (var item in AgentInventory.Items)
                if (item.Type.Equals(Eliot.Environment.ItemType.EnergyPotion) && item.Skill)
                    return true;
            return false;
        }
    }
