    ...
    public class StandardInventoryActionInterface : InventoryActionInterface
    {
        ...
        public void UseBestHealingPotion()
        {
            ...
            for (var i = 0; i < AgentInventory.Items.Count; i++)
            {
                ...
                if (item.Type.Equals(Eliot.Environment.ItemType.HealingPotion) && item.Skill && item.Value > (bestPotion == null ? -10000 : bestPotion.Value))
                    bestPotion = item;
            }
            if (bestPotion != null)
        ...
        public void UseBestEnergyPotion()
        {
            ...
            {
                var item = AgentInventory.Items[i];
                if (item.Type.Equals(Eliot.Environment.ItemType.EnergyPotion) && item.Skill && item.Value > (bestPotion == null ? -10000 : bestPotion.Value))
                    bestPotion = item;
            }
            if (bestPotion != null)
                bestPotion.Use(Agent);
